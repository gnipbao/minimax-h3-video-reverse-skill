...
本项目提供 Agent Skill、使用教程、原创示例和本地校验工具。**不包含 H3 模型，不自动生成视频，不要求 API Key。** 当前本地开发版为 **0.4.0 候选版本**：在叙事确认与事实绑定之上，新增原声继承路线及跨 Job 音轨交接；真实生成效果仍待对照，见 [验证说明](docs/validation.md)。版本说明不代表已经发布 GitHub Release。

## 0.4.0：听不懂原声，也能正确保留原声

保真反推时，源视频若确有音轨且你没要求换声，默认把**原始音轨作为独立素材沿用**。音轨内容未核听，就不编歌词、音色、配器或声效；但不会因此改配一首猜测的音乐。可选择经核实的 Ref2VA 音频提交与 `fully_copy` 关系，或在生成视觉后按原时间线只铺回一次原音轨。后者保留声音信号，**不会自动解决口型同步**，说唱和对白需要另做音画核对。完整分流见 [声音继承](references/audio-inheritance.md)。
...
| 声音边界与继承 | 听音事实、原始音轨存在、H3 参考能力与最终装配分别核对；不能听不等于不能沿用 |
...
没有实际听清的声音不要补写；源视频有音轨且不要求换声时，保留原音轨并说明实际交接方式。
```
...
**声音没有听过，就不会凭画面添加。** 未核实音轨用 `audio_status: unavailable` 说明，未确认的声音字段填 `N/A`。这表示没有编入声音事实，不表示源片静音，也不保证生成平台自动输出静音。保留原声要在可复制块外另附音频素材/装配交接；三字段文字 Prompt 自身不携带原始波形。
